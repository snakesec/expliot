...
class CANFuzz(Test):
    ...
    def execute(self):
        """Execute the test."""
        TLog.generic(
            f"Fuzz Writing to CANbus on interface({self.args.iface}), arbitration id(0x{self.args.arbitid:x}), "
        ...
        try:
            ...
            if self.args.wait < 0:
                raise ValueError(f"Illegal wait value {self.args.wait}")
            ...
            for count in range(1, self.args.count + 1):
                ...
                while datacan.find("xx") >= 0:
                    datacan = datacan.replace(
                        "xx", f"{randint(0, 0xFF):02x}", 1
                    )  # main fuzzing magic with randint
                message = CanMessage(
