...
class BaudScan(Test):
    r"""Test the available baud rate of a device.
    ...
    def __init__(self):
        ...
        self.argparser.add_argument(
            ...
            help=f"Total count of bytes to read per baud rate. Default is {DEFAULT_BYTES}",
        )
        self.argparser.add_argument(
            "-t",
            "--timeout",
            ...
            help="Read timeout, in secs, for each baud rate test. Default is {READ_TIMEOUT}",
        )
        self.argparser.add_argument(
            "-v",
            "--verbose",
    ...
    def check_baud(self, baud):
        ...
        TLog.trydo(f"Checking baud rate: {baud}")
        try:
            ...
            output["ascii_data"] = "".join(
                [
                    chr(entry)
                    for entry in output["received_data"]
            ...
            if received_length == 0:
                ...
                TLog.fail(f"\t{output['status']}")
            else:
                output["status"] = "Data received"
                output["ascii_percent"] = round(ascii_length / received_length * 100, 2)
                if self.args.verbose:
                    TLog.success(
                        f"\tdata: {output['received_data']}, ASCII: {output['ascii_data']}"
                    )
                TLog.success(
                    f"\tASCII ratio: {ascii_length}/{received_length}, {output['ascii_percent']} %"
                )
        except:
            output["exception"] = sysexcinfo()
            TLog.fail(f"\tError: {output['exception']}")
        finally:
            if sock:
                sock.close()
    ...
    def execute(self):
        """Execute the test."""
        TLog.generic(
            f"Connecting to the the serial port ({self.args.port}) timeout ({self.args.timeout})"
        )
        TLog.generic(f"Scanning for baud rates: {self.args.bauds}")
        ...
        try:
            for baud in self.args.bauds.split(","):
                ...
                if percentage_ascii == 100:
                    TLog.success(f"Found correct baud rate: {baud}")
                    return
            ...
            if best["percentage_ascii"] > 90:
                TLog.success(
                    f"Found good baud rate {best['baud_rate']} with {best['percentage_ascii']}% ASCII data"
                )
                return

            TLog.generic(
                f"Baud rate {best['baud_rate']} has max. ASCII percentage of {best['percentage_ascii']} %"
            )
        except:
