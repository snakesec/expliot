...
from expliot.plugins.dicom import DICOMPORT, REFERENCE
...
class CEcho(Test):
    ...
    def execute(self):
        """Execute the test."""
        TLog.generic(
            "Attempting to connect with DICOM server ({self.args.rhost}) on port ({self.args.rport})"
        )
        TLog.generic(
            "Using Calling AET ({self.args.aetscu}) Called AET ({self.args.aetscp})"
        )
        ...
        try:
            ...
            if assoc.is_established:
                ...
                if data_set:
                    self.output_handler(
                        cecho_response_status=f"0x{data_set.Status:04x}"
                    )
            else:
