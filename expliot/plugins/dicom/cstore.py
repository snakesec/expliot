...
class CStore(Test):
    ...
    def execute(self):
        """Execute the test."""
        TLog.generic(
            f"Attempting to send file ({self.args.file}) to DICOM server ({self.args.rhost}) on port ({self.args.rport})"
        )
        TLog.generic(
            f"Using Calling AET ({self.args.aetscu}) Called AET ({self.args.aetscp})"
        )
        file = None
        assoc = None
        try:
            ...
            if self.args.lport != 0:
                TLog.generic(f"Using source port number ({self.args.lport})")
                if (self.args.lport < 1024) and (os.geteuid() != 0):
                    TLog.fail("Oops! Need to run as root for privileged port")
                    raise ValueError(
                        f"Using privileged port ({self.args.lport}) without root privileges"
                    )
            ...
            if assoc.is_established:
                ...
                if status.Status != 0x0000:
                    success = False
                    reason = (
                        f"C-STORE Failed to store file (status=0x{status.Status:04x})"
                    )
                    TLog.fail(reason)
                    self.result.setstatus(passed=False, reason=reason)
                self.output_handler(
                    cstore_status=f"0x{status.Status:04x}", cstore_success=success
                )
            else:
