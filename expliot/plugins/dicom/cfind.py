...
from expliot.plugins.dicom import (
    DEFAULTMODEL,
    DICOMPORT,
    MODELS,
    REFERENCE,
...
class CFind(Test):
    ...
    def __init__(self):
        ...
        self.argparser.add_argument(
            ...
            default=DICOMPORT,
            type=int,
            help=f"The port number of the target DICOM Server (SCP). "
            f"Default is {DICOMPORT}",
        )
        self.argparser.add_argument(
            "-q",
            "--lport",
        ...
        self.argparser.add_argument(
            ...
            help=f"Specify the information model to use for C-FIND. "
            f"Options are P, S, W and O for patient root, study "
            f"root, modality worklist and patient/study only (retired) "
            f"respectively. Default is {DEFAULTMODEL}",
        )
    ...
    def execute(self):
        ...
        if self.args.model not in MODELS:
            self.result.setstatus(
                passed=False, reason=f"Unknown model ({self.args.model}) specified"
            )
            return
        TLog.generic(
            f"Attempting to search for patient ({self.args.name}) on DICOM server ({self.args.rhost}) on port ({self.args.rport})"
        )
        TLog.generic(
            "Using Calling AET ({self.args.aetscu}) Called AET ({self.args.aetscp}) Information model ({self.args.model})"
        )
        assoc = None
        try:
            ...
            if self.args.lport != 0:
                TLog.generic("Using source port number ({self.args.lport})")
                if (self.args.lport < 1024) and (os.geteuid() != 0):
                    TLog.fail("Oops! Need to run as root for privileged port")
                    raise ValueError(
                        "Using privileged port ({self.args.lport}) without root privileges"
                    )
            ...
            if assoc.is_established:
                ...
                if responses is not None:
                    for status, identifier in responses:
                        ...
                        statdict = {
                            "cfind_query_status": f"0x{status.Status:04x}",
