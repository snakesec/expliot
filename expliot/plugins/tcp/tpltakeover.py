...
from expliot.core.vendors.tplink import TPL_PORT, crypto
...
class TPLinkTakeover(Test):
    ...
    def __init__(self):
        ...
        self.argparser.add_argument(
            "-r", "--rhost", required=True, help="IP address of TPlink Smart device"
        )
        ...
        self.argparser.add_argument(
            ...
            default=TPL_PORT,
            type=int,
            help=f"Port number of the smart device service. Default is {TPL_PORT}",
            required=True,
        )
        ...
        try:
            ...
            if not data_tp:
                self.result.setstatus(
                    passed=False, reason="Empty or no response received"
                )
            else:
                ...
                # Step 6 : Check for reason of failure
                if json_data.get("system") and json_data.get("system").get("err_msg"):
                    reason = json_data.get("system").get("err_msg")
                    self.result.setstatus(passed=False, reason=reason)
                elif (
                    json_data.get("system")
                    and not json_data.get("system").get("err_msg")
                    and json_data.get("system").get("set_relay_state")
                    and json_data.get("system").get("set_relay_state").get("err_msg")
                ):
                    reason = (
                        json_data.get("system").get("set_relay_state").get("err_msg")
                    )
                    self.result.setstatus(passed=False, reason=reason)
                elif (
                    json_data.get("system")
            ...
            self.result.setstatus(
                passed=False,
                reason=f"Could not connect to host {self.args.rhost}:{str(self.args.rport)}",
            )
        finally:
            if sock_tcp:
                sock_tcp.close()
