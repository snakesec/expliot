...
class BleCharFuzz(Test):
    ...
    def execute(self):
        """Execute the test."""
        TLog.generic(
            f"Fuzzing the value ({self.args.value}), iterations ({self.args.iter}) for handle ({hex(self.args.handle)}) on BLE device ({self.args.addr})"
        )
        try:
            ...
            for _ in range(self.args.iter):
                ...
                while value.find("xx") >= 0:
                    value = value.replace("xx", f"{randint(0, 0xFF):02x}", 1)  # nosec

                self.output_handler(
                    tlogtype=TLog.TRYDO,
                    msg=f"Writing the fuzzed value ({value})",
                    logkwargs=LOGNO,
                    fuzzvalue=value,
                )
