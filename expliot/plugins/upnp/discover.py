...
class Discover(Test):
    ...
    def __init__(self):
        ...
        self.argparser.add_argument(
            ...
            type=int,
            help=f"Timeout in seconds for each device type. "
            f"Default is {DEFAULT_UPNP_TIMEOUT} seconds",
        )
    ...
    def log_callback(self, cbdata, robj, rlevel, key=None, value=None):
        """Execute the recursive iteration of dict.

        It logs everything except for response data and description (XML)
        unless the user runs the plugin in verbose mode.

        Args:
            cbdata: Date of the callback
            robj: Object
            rlevel: Level
            key: key
            value: Value
        ...
        if key in ("description", "response") and (not self.args.verbose):
            return
        if robj.__class__ == dict and value.__class__ in (dict, list):
            TLog.success(f"{spaces}{key}:")
        elif key:
            TLog.success(f"{spaces}{key}: {value}")
        else:
            TLog.success(f"{spaces}{value}")
