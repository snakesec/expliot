...
from expliot.core.protocols.hardware.i2c import (
    I2cController,
    I2cNackError,
)
    ...
    def execute(self):
        ...
        try:
            ...
            for address in range(i2c.HIGHEST_I2C_ADDRESS + 1):
                ...
                try:
                    port.read(0)
                    self.output_handler(address_found=f"{hex(address)}")
                    passed = passed + 1
                except I2cNackError:
                    fail = fail + 1
            self.output_handler(total_found=passed, total_not_found=fail)
        except:
