...
class I2cEepromWrite(Test):
    ...
    def execute(self):
        """Execute the test."""
        TLog.generic(
            f"Writing data to i2c eeprom at address({self.args.addr}) using device({self.args.url})"
        )
        device = None
        try:
            ...
            if self.args.rfile:
                ...
                with open(self.args.rfile, "r+b") as input_file:
            ...
            TLog.trydo(
                f"Writing {length_data} byte(s) at start address {start_address}"
            )
