...
class CoapPost(Test):
    ...
    def __init__(self):
        ...
        self.argparser.add_argument(
            ...
            "--path",
            default=ROOTPATH,
            help=f"Resource URI path of the POST request. Default "
    ...
    def execute(self):
        """Execute the test."""
        TLog.generic(
            f"Sending POST request to URI Path ({self.args.path}) "
            f"to CoAP Server {self.args.rhost} port {self.args.rport} with data ({self.args.data})"
        )
        try:
            client = CoapClient(self.args.rhost, port=self.args.rport)
            response = client.post(path=self.args.path, payload=self.args.data.encode())
