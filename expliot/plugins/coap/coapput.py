...
class CoapPut(Test):
    ...
    def __init__(self):
        ...
        self.argparser.add_argument(
            ...
            "--path",
            default=ROOTPATH,
            help=f"Resource URI path of the PUT request. Default "
    ...
    def execute(self):
        """Execute the test."""
        TLog.generic(
            f"Sending PUT request to URI Path ({self.args.path}) "
        ...
        try:
            ...
            response = client.put(path=self.args.path, payload=self.args.data.encode())
