...
class AwsIotPub(Test):
    ...
    def __init__(self):
        ...
        self.argparser.add_argument(
            ...
            type=int,
            help=f"MQTT operation/connection timeout in seconds. "
            f"Default is {DEFAULT_AWSIOT_TIMEOUT} secs",
        )
    ...
    def execute(self):
        ...
        TLog.generic(
            f"Publishing message on topic ({self.args.topic}) to AWS IoT endpoint ({self.args.rhost}) on port "
            f"({self.args.rport})"
        )
        ...
        try:
            ...
            if self.args.user and self.args.passwd:
                TLog.trydo(
                    f"Using authentication (username={self.args.user})(password={self.args.passwd})"
                )
            thing.connect()
            thing.publish(self.args.topic, self.args.msg, 1)
            TLog.success(
                f"Message ({self.args.msg}) published on topic ({self.args.topic})"
            )
        except:
