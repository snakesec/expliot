...
from expliot.core.protocols.internet.mdns import (
...
class Discover(Test):
    ...
    def __init__(self):
        ...
        self.argparser.add_argument(
            ...
            f"Default is {DEFAULT_MDNS_TIMEOUT} seconds.",
        )
    ...
    def execute(self):
        ...
        if self.args.device:
            if self.args.device not in service_names:
                self.result.setstatus(
                    passed=False, reason="Unknown device type specified"
                )
                return
        ...
        self.output_handler(total_devices_discovered=counter)
