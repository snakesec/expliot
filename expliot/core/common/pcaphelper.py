...
class PcapDumper:
    ...
    def __init__(self, datalink, filename):
        ...
        if self.__filename is not None and isinstance(self.__filename, str):
            with open(self.__filename, mode="wb") as __file:
                # Write global header
                line = struct.pack(
                    "IHHIIII",
                    PCAPH_MAGIC_NUM_BE,
    ...
    def write_to_pcapfile(self, pcapframe):
        ...
        """
        with open(self.__filename, mode="ab") as __file:
            __file.write(pcapframe)
            __file.flush()


def wireshark_dump_reader(filename):
    """Opens pcap file for reading.

    Read wireshark global header
    Identify file endienness and read next packet accordingly

    :param filename: pcap file name to be rad from disk

    TODO: Test magic number for BE machine
    """
    with open(filename, mode="rb") as __file:
        # Read first pcap file header
        pcaphdr = __file.read(WIRESHARK_GLB_HDR_LEN)
        ...
        pcap_hdr_t = ["", "", "", "", "", "", ""]
        pcap_hdr_t = struct.unpack(f"{__endianness}IHHIIII", pcaphdr)

        if pcap_hdr_t[1] != PCAPH_VER_MAJOR and pcap_hdr_t[2] != PCAPH_VER_MINOR:
            raise ValueError("Unsupported pcap file version")

        _hdr_snaplen = pcap_hdr_t[5]
        # _hdr_datalink = pcap_hdr_t[6]

        while True:
            packet_header = __file.read(WIRESHARK_PKT_HDR_LEN)
            ...
            pcaprec_hdr_t = ["", "", "", ""]
            pcaprec_hdr_t = struct.unpack(f"{__endianness}IIII", packet_header)

            if (
                pcaprec_hdr_t[2] > pcaprec_hdr_t[3]
                or pcaprec_hdr_t[2] > _hdr_snaplen
                or pcaprec_hdr_t[3] > _hdr_snaplen
            ):
                raise ValueError("Invalid pcap packet header")

            # Header is valid, now read the packet with incl_len
            packet = __file.read(pcaprec_hdr_t[2])
