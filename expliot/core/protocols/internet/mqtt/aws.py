...
class AwsMqttClient(AWSIoTMQTTClient):
    ...
    It implements easy configuration method, default callbacks and connection
    state check. If you need customized configuration and callbacks, directly
    use AWSIoTMQTTClient instead.
    """
    ...
    def easy_disconnect(self):
        ...
        Works with a check for valid connection before disconnecting. For
        resource cleanup we need to make sure
    ...
    def easy_config(self, **kwargs):
        ...
        Collects and provides simple callbacks to monitor connection and
        subscribe.
