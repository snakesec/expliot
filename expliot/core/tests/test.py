...
from expliot.core.common import bstr, recurse_list_dict
...
class TCategory(namedtuple("TCategory", "tech, iface, action")):
    ...
    _actions = [ANALYSIS, COMPLIANCE, DISCOVERY, EXPLOIT, FUZZ, RECON]

    def __init__(self, tech, iface, action):
        """Initialize the test category."""
        if tech not in TCategory._tech:
            raise AttributeError(f"Unknown technology for category - ({tech})")
        if iface not in TCategory._interfaces:
            raise AttributeError(f"Unknown interface for category - ({iface})")
        if action not in TCategory._actions:
            raise AttributeError(f"Unknown action for category - ({action})")
        super().__init__()
...
class TTarget(namedtuple("TTarget", "name, version, vendor")):
    ...
    AWS = "aws"
    TP_LINK_IOT = "tpliot"
    _name = [AWS, GENERIC, LINUX, TP_LINK_IOT]
    ...
    TP_LINK = "tplink"
    _vendor = [AMAZON, GENERIC, LINUX, TP_LINK]
...
class TResult:
    ...
    def getresult(self):
        ...
        """
        return {
            "status": 0 if self.passed is True else 1,
            "reason": self.reason,
            "output": self.output,
        }
...
class TLog:
    ...
    def print(cls, prefixtype, message):
        ...
        try:
            ...
            pre = cls._errprefix
        print(f"{pre} {message}", file=cls._file)
...
class Test:
    ...
    def output_dict_iter(self, cblog, robj, rlevel, key=None, value=None):
        ...
        if robj.__class__ == dict and (
            ...
            if cblog["logkwargs"] == LOGPRETTY:
                TLog.print(cblog["tlogtype"], f"{spaces}{key}:")
        else:
            strval = value
            isbval = False
            ...
            if key:
                if cblog["logkwargs"] == LOGPRETTY:
                    TLog.print(
                        cblog["tlogtype"], f"{spaces}{key}: {strval}"
                    )
                if isbval:
                    robj[key] = strval
            else:
                if cblog["logkwargs"] == LOGPRETTY:
                    TLog.print(cblog["tlogtype"], f"{spaces}{strval}")
                if isbval:
                    robj[robj.index(value)] = strval

    def output_handler(
        self, tlogtype=TLog.SUCCESS, msg=None, logkwargs=LOGPRETTY, **kwargs
    ):
        ...
        if not kwargs:
            # empty dict
            return
        cblog = {"tlogtype": tlogtype, "logkwargs": logkwargs}
    ...
    def _assertpriv(self):
        ...
        if self.needroot and geteuid() != 0:
            raise PermissionError(
                f"Need root privilege to execute the plugin ({self.id})"
            )
    ...
    def _setid(self):
        ...
        self.id = f"{self.category.tech}.{self.target.name}.{self.name}".lower()
    ...
    def _logstatus(self):
        ...
        if self.result.passed:
            TLog.success(f"Test {self.id} passed")
        else:
            TLog.fail(f"Test {self.id} failed. Reason = {self.result.reason}")
