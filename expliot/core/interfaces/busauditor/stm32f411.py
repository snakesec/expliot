...
class STM32F411(BusAuditorServices):
    ...
    def read_supported_service(self):
        ...
        resp = self.usb_cntrl_read(
            self.SRV_GET_SERVICES, data_or_wlength=self.SRV_GET_SERVICES_RESP_LEN
        )
        ...
        if (
            ...
            if resp[self.SRV_RESP_BYTE_4] & self.SRV_SWD_SCAN_BIT:
                self.set_device_service(self.SWD_SCAN, True)

            if resp[self.SRV_RESP_BYTE_4] & self.SRV_UART_SCAN_BIT:
                self.set_device_service(self.UART_SCAN, True)

            if resp[self.SRV_RESP_BYTE_4] & self.SRV_I2C_SCAN_BIT:
                self.set_device_service(self.I2C_SCAN, True)
    ...
    def device_jtag_idcode_scan(self, start, end, volts, include_trst=False):
        ...
        self.usb_cntrl_write(
            brequest=service, wvalue=0, windex=0, data_or_wlength=indata
        )
    ...
    def __extract_jtag_idcode_pins_data(idcode_list, pattern_dict):
        ...
        Sources are pattern_dict and pins from
        ...
        for _, value in enumerate(tmp_id_list):
            ...
            for _, idcode in enumerate(idcodes):
                device_dict = {}
                device_dict["jtag_idcode"] = f"0x{idcode:08x}"
    ...
    def device_jtag_scan(self, start, end, volts, include_trst=False):
        ...
        idcode_list = self.device_jtag_idcode_scan(start, end, volts, include_trst)

        time.sleep(2)

        pattern_dict = self.device_jtag_pattern_scan(start, end, volts, include_trst)
    ...
    def __build_swd_idcode_dict(resp_array):
        ...
        for resp in resp_array:
            ...
            if idcode not in idcode_dist:
                idcode_dist.update({idcode: {"swclk": data[0], "swdio": data[1]}})

        device_list = []
        for idcode, value in idcode_dist.items():
            device_dict = {}
            device_dict["swd_idcode"] = f"0x{idcode:08x}"
    ...
    def device_uart_tx_scan(self, start, end, volts):
        ...
        self.usb_cntrl_write(
            brequest=self.SRV_UART_TX_SCAN, wvalue=0, windex=0, data_or_wlength=indata
        )
    ...
    def __build_i2c_addr_dict(resp_array):
        ...
        for resp in resp_array:
            ...
            for _, addr in enumerate(addr_list):
                device_dict = {}
                device_dict["i2c_addr"] = f"0x{addr:02x}"
    ...
    def device_i2c_scan(self, start, end, volts):
        ...
        self.usb_cntrl_write(
            brequest=self.SRV_I2C_ADDR_SCAN, wvalue=0, windex=0, data_or_wlength=indata
        )
    ...
    def __read_service_response(self, service):
        ...
        while True:
            ...
            if (
                ...
                raise ValueError(f"Service Error - {self._srv_err[resp[2]]}")
            if (
                len(resp) > 3
                and resp[0] == self.SERVICE_RESP_BIT | service
                and resp[2] == 0
                ...
                raise ValueError(f"Unknow Error - {resp[2]}")
